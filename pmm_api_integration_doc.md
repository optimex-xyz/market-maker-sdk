...
The provided documentation is an alternative implementation guide for integrating Private Market Makers (PMMs) with BitFi's solver backend using Solver API directly. It details the endpoints and request/response formats that PMMs need to implement to communicate with the solver backend for providing quotes, committing to trades, and submitting settlement transactions.
